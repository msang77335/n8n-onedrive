      ...
      data.TRACKING_ORDERS.forEach((order: any, index: number) => {
        ...
        if (order.TRACKINGS && Array.isArray(order.TRACKINGS)) {
          ...
          if (firstTracking) {
            ...
            // Show location link if there's post office info
            const tenBuuCuc = firstTracking.TEN_BUUCUC_DI || firstTracking.TEN_BUUCUC_DEN || '';
            const hasLocationInfo = (tenBuuCuc?.includes('Bưu cục')) ||
              (timelineContent.includes('Bưu cục') && (firstTracking.LATITUDE_BUUCUC && firstTracking.LONGITUDE_BUUCUC));
